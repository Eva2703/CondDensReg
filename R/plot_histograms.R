...
#' 
#' \code{\link[=plot_single_histo]{plot_single_histo()}} can display the resulting histogram or bar plot of a subset of data which was prepared by \code{\link[=preprocess]{preprocess()}} which contains a vector of (weighted) counts via a histogram on \eqn{I\setminus D} and (weighted) counts on \eqn{D} where \eqn{I} is the interval of the continuous domain and \eqn{D} the set of discrete values. It is meant to take only one group, i.e., one of the different covariate combinations, into account. If no sample weights were included in the data in the \code{\link[=preprocess]{preprocess()}}-step, the unweighted counts are used instead of weighted counts.
#'
#' @encoding UTF-8
...
#' @param dta \code{data.table}-object which is an object of the sub-class \code{histogram_count_data}, i.e., the output of the \code{\link[=preprocess]{preprocess()}}-function. Subset in such a way, that only one covariate combination, i.e. one \code{group_id} is contained.
...
#'
#' @return The returned object is a plot object specified as explained above as a histogram or bar plot of frequencies or absolute counts. The display of mixed case count (histogram) data is as considered in the work of Maier et al. (2023).
#' 
#' @examples 
#' set.seed(101)
#' 
...
plot_single_histo <- function(dta, case = "all", abs = FALSE, main = NULL, 
  ...
  # if not specified use name of density_var as x-axis label
  if (is.null(xlab)) {
    xlab <- response_name
  }
  ...
  if (isFALSE(abs)) {
    ...
    if (case == "all") {
      ...
      if (hist) {
        ...
        barplot(plot_hist$height, main = main, xlab = xlab, ylab = ylab, 
                width = plot_hist$Delta,space = 0,ylim = ylim,
                ...)
        # add bars of discrete values
        segments(x0 = x_values_discrete-min(plot_hist[,..response_name])+1/2*plot_hist$Delta[1], y0 = ylim[1], x1 = x_values_discrete-min(plot_hist[,..response_name])+1/2*plot_hist$Delta[nrow(plot_hist)],
      ...
      if (!hist) {
        ...
        barplot(plot_hist$freq, main = main, xlab = xlab, ylab = ylab, 
                width = plot_hist$Delta,space = 0,ylim = ylim,
                ...)
        # add bars of discrete values
        segments(x0 = x_values_discrete-min(plot_hist[,..response_name])+1/2*plot_hist$Delta[1], y0 = ylim[1], x1 = x_values_discrete-min(plot_hist[,..response_name])+1/2*plot_hist$Delta[nrow(plot_hist)],
    ...
    if (case == "discrete") {
      ...
      if (hist) {
        ...
        barplot(plot_hist$freq, main = main, xlab = xlab, ylab = ylab, 
                ...
                ...)
        # add bars of discrete values
        segments(x0 = x_values_discrete, y0 = ylim[1], x1 = x_values_discrete,
                 y1 = y_values_discrete,col = col_discrete)
  ...
  if (!isFALSE(abs)) {
    ...
    if (case == "all") {
      ...
      if (!hist) {
        ...
        barplot(plot_hist$weighted_counts,main = main, xlab = xlab, ylab = ylab, 
                width = plot_hist$Delta,space = 0,ylim = ylim,
                ...)
        # add bars of discrete values
        segments(x0 = x_values_discrete-min(plot_hist[,..response_name])+1/2*plot_hist$Delta[1], y0 = ylim[1], x1 = x_values_discrete-min(plot_hist[,..response_name])+1/2*plot_hist$Delta[nrow(plot_hist)],
...
#' @import cowplot
#' @import grid
#' @param dta \code{data.table}-object which is also an object of the sub-class \code{histogram_count_data}, i.e., the output of the \code{\link[=preprocess]{preprocess()}}-function. Subset in such a way, that only one covariate combination, i.e. one \code{group_id} is contained.
#' @param selected_groups Numeric vector of \code{group_id}s for which the respective plots will be created and saved. If missing (\code{NULL}) plots of all groups within the range of the minimum and maximum \code{group_id} will be saved.
#' @param single \code{logical} indicating if all plots are saved as single PDFs if \code{single=TRUE} or jointly in one document with four plots per page if \code{single=FALSE}. If missing (\code{NULL}), the default is \code{TRUE}.
#' @param path Directory path where resulting plots will be saved. If not specified by the user then the storage directory is the subdirectory "Images" of the current working directory. If this does not yet exist, it will be created.
#' @param case Plotting case as a \code{character}-object: can be specified as "all", "continuous" or "discrete". Determines the components of the data which will be plotted: "all" leads to a plot of the complete data, "continuous" only plots the bins displaying data of the continuous domain \eqn{I} and "discrete" shows only bins displaying the counts of the respective discrete values in \eqn{D}. Please note that the plotting case is not the same as the specification of the case in \code{\link[=preprocess]{preprocess()}} where the bins and discrete values are defined. If the case is not "all" and \code{hist} is TRUE, the sum of the areas under the bins and the discrete bars is not one. If missing (\code{NULL}), the default is "all".
#' @param abs \code{logical} which determines if the resulting plot shows the absolute, (weighted) counts (\code{abs=TRUE}) or the relative frequencies of the weighed counts (\code{abs=FALSE}). If missing (\code{NULL}), the default is \code{FALSE}.
#' @param main Optional \code{character} passing a user-defined main title for the resulting plot. If missing (\code{NULL}), the main will be generated automatically, see also \code{automatic_main}.
#' @param automatic_main \code{logical} determining the automatic generation of a main title for the plot which contains the number of bins, the plot case defined by \code{case}, the values of the respective covariate combination and the group_id. If \code{TRUE} and a user-defined \code{main} is passed, the automatic main is overwriting the passed main. If missing (\code{NULL}), the default is \code{TRUE}.
...
#' @param ylab Optional \code{character} passing a user-defined title of the y-axis. If missing (\code{NULL}), the axis title contains the information if absolute values or frequencies are shown in a histogram (for the continuous components) or a bar plot. 
#' @param col_discrete Name or hexadecimal RGB triplet of a color contained in base R. Determines the color of the bars of the discrete values. If missing (\code{NULL}), the default color is "red". It is recommended to use a color contrasting the black outlines of the bins of the contunuous domain especially if one discrete value lies exactly at a bin border. 
#' @param file_names Vector of file names can be passed by the user. If \code{single=TRUE} the number of file names has to be the same as the number of selected groups. If \code{single=FALSE} only one file name can be passed. If missing (\code{NULL}), the names of the single plots will be their \code{group_id} with the prefix "hist_". The name of the file if \code{single=FALSE} is "all.pdf". A warning occurs if \code{file_names_stem} is not \code{NULL} because only one of the two parameters \code{file_names_stem} and \code{file_names} can be specified.
#' @param file_names_stem \code{character} which replaces the prefix "hist_" in the automatically generated file name if \code{file_names=NULL}. If missing (\code{NULL}), the names of the single plots will be their \code{group_id} with the prefix "hist_". The name of the file if \code{single=FALSE} is "all.pdf". A warning occurs if \code{file_names} is not \code{NULL} because only one of the two parameters \code{file_names_stem} and \code{file_names} can be specified.
#' @param plot_width Value for the plot width in the case of \code{single=TRUE}. For \code{single=FALSE} the plot width is not adjustable. If missing (\code{NULL}), the default value if \code{single=TRUE} is as for the \code{pdf()}-function 7 inches.
#' @param plot_height Value for the plot height in the case of \code{single=TRUE}. For \code{single=FALSE} the plot height is not adjustable. If missing (\code{NULL}), the default value if \code{single=TRUE} is as for the \code{pdf()}-function 7 inches.
#' @param hist \code{logical} which determines if the resulting plot is a histogram or a simple bar plot of the data. If \code{case="all"} and \code{hist=TRUE}, the areas under the bins and the height of discrete bars times the respective weight of the discrete dirac measure sum up to one. Otherwise the height of the bins is directly referring to the frequency or the (weighted) counts of the respective bin without taking the bin width or the dirac measure into account. If missing (\code{NULL}), the default is \code{TRUE}.
#' @param ... Other parameters which can be passed to the respective plotting function specifying plotting parameters
#'
...
#'   )
#' 
save_plots <- function(dta, selected_groups = unique(dta$group_id), single = TRUE, path = "./Images/", 
                       case = "all", main = NULL, automatic_main = TRUE, 
                       ylim = NULL, xlab = NULL, ylab = NULL, 
                       col_discrete = "red", file_names = NULL, 
                       file_names_stem = NULL, plot_width = ifelse(single, 7, 480), #default values for width and height in pdf()/png() 
  ...
  if (single) {
    for (k in selected_groups) {
      ...
      if (!is.null(file_names)) {
        ...
        pdf(paste0(path, file_names[i], ".pdf"),
            width = plot_width,
            height = plot_height)
      }
  ...
  # if single is FALSE, save all plots in one pdf file, 4 per page
    ...
    if (is.null(file_names)) {
      pdf(paste0(path, "all", ".pdf"))
    }
    # or use passed file name
    else{
      pdf(paste0(path, file_names, ".pdf"))
    }
...
#' @import manipulate
#' @importFrom Rdpack reprompt
#' @param dta \code{data.table}-object which is also an object of the sub-class \code{histogram_count_data}, i.e., the output of the \code{\link[=preprocess]{preprocess()}}-function with multiple covariate combinations, i.e. \code{group_id}s, contained.
...
#' @encoding UTF-8
#' @param dta \code{data.table}-object which is an object of the sub-class \code{histogram_count_data}, i.e., the output of the \code{\link[=preprocess]{preprocess()}}-function with multiple covariate combinations, i.e. \code{group_id}s, contained.
#' @return Interactive plot displaying all histograms for the contained groups where the respective \code{group_id} is selectable with a slider. The display of mixed case count (histogram) data is as considered in the work of Maier et al. (2023).
#' @export
#' @references Maier, E. M., Fottner, A., Stöcker, A., Okhrin, Y., & Greven, S. (2023). Conditional density regression for individual-level data.
#'
#' @examples 
...
plot.histogram_count_data <- function(dta) {
  tmp <- as.data.table(dta)
